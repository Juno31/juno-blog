...
function Home() {
  ...
  useEffect(function () { // 배경화면 줌인 이벤트 리스너
    ...
    const zoomEvent = async function (e) { // 첫 랜더링 시 휠 효과
      var imgElement = document.getElementById('background');
      var wrapper = document.getElementById('wrapper');
      var headerWrapper = document.getElementById('header-wrapper')

      if (e.deltaY < 0 && initialScale > 1.2) { // 줌 아웃
        console.log("working", e.deltaY, '', initialScale)
        headerWrapper.style.opacity = `0`
        wrapper.style.opacity = `1`; 
        wrapper.style.zIndex = `1`
        imgElement.style.transform = `scale(${(initialScale-= 0.1)})`
        imgElement.style.bottom = `${initialBottom += 5}px`

        
      } else if (e.deltaY > 0 && initialScale < 2) { // 줌 인
        wrapper.style.zIndex = `1`
        initialScale += 0.1
        initialBottom -= 5
